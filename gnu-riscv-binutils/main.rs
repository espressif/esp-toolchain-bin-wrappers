    ...
    for arg in argv {
        ...
        if let Some(value) = arg.strip_prefix(MARCH_ARG_PREFIX) {
            ...
            if value.contains("xesppie") {
                ...
                esp_debug_trace!("tool_suffix=\"{}\" based on xesppie: \"{}\"", tool_suffix, arg);
            }
        }
        if let Some(value) = arg.strip_prefix('@') {
            // filepath may be surrounded by quotes and double quotes, so strip them
            esp_debug_trace!("processing flags file=\"{}\"", value);
            ...
            match std::fs::read_to_string(value) {
                Ok(content) => {
                    ...
                    if let Some(suffix) = parse_args_for_suffix(&flags) {
                        tool_suffix = suffix;
                        esp_debug_trace!("tool_suffix=\"{}\" based on flags file \"{}\"", tool_suffix, value);
                    }
                }
                Err(e) => {
                    esp_debug_trace!("Warning: Failed to read flags file \"{}\": {}", value, e);
